...
The following is example code to use the API using python. Then API is designed to return data to the user in a convienient manner to be used within existing software builds.
...
```
##### Returned value: Success
If the post is successful Access Token and Refresh token as a string. Save these for use later.
```JSON
{"message": "user mynewusername was created", "access_token": "***********", "refresh_token": "*******"}
```
...
Returned values:
Json: This contains all of the data extracted from the invoice. Also tables are represented in 2 formats - standardised header and original headers.
